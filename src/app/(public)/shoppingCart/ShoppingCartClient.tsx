...
const ShoppingCartClient = () => {
    ...
    useEffect(() => {
        ...
        // Track InitiateCheckout when user visits the cart page with items
        if (cart?.cartItems?.length > 0) {
            const totalValue = cart.cartTotalAmount;
            const contentIds = cart.cartItems.map((item: CartItem) => item.id);
            const numItems = cart.cartItems.reduce((total: number, item: CartItem) => {
                        ...
                        <div className="w-full md:max-w-xs flex-shrink-0 mb-6 md:mb-0">
                            <div className="bg-white rounded-lg p-6 flex flex-col gap-4 border border-gray-300">
                                ...
                                <button 
                                    ...
                                    className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-lg shadow-md transition mb-2"
                                >
                                    Checkout
                                </button>
                                <button 
                                    onClick={() => navigate.push('/')} 
                                    className="w-full bg-white border border-indigo-600 text-indigo-600 font-semibold py-2 rounded-lg hover:bg-indigo-50 transition"
                                >
